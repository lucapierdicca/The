...
def dump_map_ground_truth(parsed_ggb):
    ...
    with open(f"data/train/train_map_2_ground_truth.pickle", 'wb') as f:
        pickle.dump(ground_truth, f)

def dump_map_wall_boundary_vertices(wall_boundary_vertices):
    with open(f"data/train/train_map_2_wall_boundary_vertices.pickle", 'wb') as f:
        pickle.dump(wall_boundary_vertices, f)

ggb_file = "ggb/train_map/geogebra.xml"
...
for i,nr in enumerate(n_robot):
    ...
    for j,s in enumerate(runs_seeds[i]):
        ...
        params.attrib["file_name"] =  f"data/train/unstructured_occluded_3.csv"
